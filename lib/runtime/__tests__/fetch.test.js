...
const { Request, Response, Headers, bindCfProperty } = require("../fetch");
...
describe("fetch", () => {
  ...
  test("fetch deletes host header from header object", async () => {
    ...
    await shimFetch("https://www.google.com", {
      headers: new Headers({ host: "test.com" }),
    });

    const headers =
      fetchMock.mock.calls[0][0].headers ||
  ...
  test("fetch deletes host header from Request with object", async () => {
    fetchMock.mockResponse("");
    await shimFetch(
      new Request("https://www.google.com", { headers: { host: "test.com" } })
    );
