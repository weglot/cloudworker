...
describe("cloudflare cache policy", () => {
  test("cache policy storable only for GET requests", () => {
    const getReq = new Request("https://dollarshaveclub.com/", {
      method: "GET",
    });
    const postReq = new Request("https://dollarshaveclub.com/", {
      method: "POST",
    });
  ...
  test("cache policy storable for any request method with ignoreMethod option", () => {
    ...
    expect(
      new CFCachePolicy(getReq, res, { ignoreMethod: true }).storable()
    ).toBe(true);
    expect(
      new CFCachePolicy(postReq, res, { ignoreMethod: true }).storable()
    ).toBe(true);
  });
  ...
  test("cache policy storable for response with set-cookie and private=set-header cache-control", () => {
    ...
    const res = new Response("test", {
      headers: {
        "set-cookie": "yes",
        "cache-control": "max-age=200, private=set-cookie",
      },
    });
