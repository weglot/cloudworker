...
class CloudflareCachePolicy extends CachePolicy {
  constructor(req, res, opts = {}) {
    const policyReq = CloudflareCachePolicy.cachePolicyRequestFromRequest(
      req,
      opts.ignoreMethod === true
    );
    const policyRes = CloudflareCachePolicy.cachePolicyResponseFromResponse(
      res
    );
  ...
  static cachePolicyResponseFromResponse(res) {
    ...
    const pruneSetCookie = cacheControl
      .toLowerCase()
