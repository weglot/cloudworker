...
class Cloudworker {
  constructor(
    ...
    const cacheFactory = enableCache
      ? new CacheFactory()
      : new StubCacheFactory();
  ...
  async _handle(req, res) {
    ...
    if (req.method !== "GET" && req.method !== "HEAD") {
      ...
      if (
        ...
        // otherwise, read body into buffer
        const temp = new runtime.Request(url, {
          headers: req.headers,
          body: req,
          method: req.method,
        });
    ...
    const request = new runtime.Request(url, {
      headers: req.headers,
      body: body,
      method: req.method,
    });
  ...
  _logRequest(request, response, start, end) {
    ...
    this._logDebug(
      `${client} - - [${date}] ${targetHost} "${request.method} ${request.url} HTTP/${request.httpVersion}" ${response.statusCode} ${duration}`
    );
  }
