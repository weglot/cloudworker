...
module.exports.extractKVBindings = (kvSetFlags, kvFileFlags) => {
  ...
  for (const flag of kvSetFlags) {
    ...
    if (comps.length < 2 || comps[1].split("=").length < 2) {
      throw new Error(
        "Invalid kv-set flag format. Expected format of [variable].[key]=[value]"
      );
    }
...
const extractKVPaths = (kvFileFlags) => {
  ...
  for (const flag of kvFileFlags) {
    ...
    if (flag.length < 2) {
      throw new Error(
        "Invalid kv-file flag format. Expected format of [variable]=[value]"
      );
    }
...
module.exports.parseWasmFlags = (wasmFlags) => {
  ...
  for (const flag of wasmFlags) {
    ...
    if (comps.length !== 2) {
      throw new Error(
        "Invalid wasm flag format. Expected format of [variable=path]"
      );
    }
