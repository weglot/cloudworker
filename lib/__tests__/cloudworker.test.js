...
describe("cloudworker", () => {
  test("throws on invalid script", async () => {
    expect(() => {
      new Cloudworker();
    }).toThrow(new TypeError("worker script must be a string")); // eslint-disable-line no-new
    expect(() => {
      new Cloudworker(12);
    }).toThrow(new TypeError("worker script must be a string")); // eslint-disable-line no-new
  });
  ...
  test("dispatch binds cf property", async (done) => {
    ...
    expect(res.headers.get("x-tlscipher")).toEqual(
