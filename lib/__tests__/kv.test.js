...
describe("kv", () => {
  ...
  test("KeyValueStore arrayBuffer", async () => {
    ...
    expect(valueArrayBuffer).arrayBufferIsEqual(
      Uint8Array.from(Buffer.from("world")).buffer
    );
  });
  ...
  test("KeyValueStore delete throws on nonexistent key", async () => {
    ...
    expect(() => {
      kv.delete("hello");
    }).toThrowError(new Error("HTTP DELETE request failed: 404 Not Found"));
  });
