...
describe("cloudworker-e2e", async () => {
  ...
  test("wasm", async (cb) => {
    const script = utils.read(
      path.join(__dirname, "fixtures/worker-wasm-simple.js")
    );
    const bindings = {
      Wasm: await wasm.loadPath(path.join(__dirname, "fixtures/simple.wasm")),
    };
  ...
  test("kv", async (cb) => {
    const script = utils.read(
      path.join(__dirname, "fixtures/worker-kv-simple.js")
    );
  ...
  test("streams", async (cb) => {
    const script = utils.read(
      path.join(__dirname, "fixtures/worker-streams.js")
    );
    const server = new Cloudworker(script).listen(8080);
    ...
    expect(res.status).toEqual(200);
    expect(Buffer.from(res.data)).toEqual(
      Buffer.from(new Uint16Array([10, 20]))
    );
    await server.close();
    cb();
  });
  ...
  test("post upstream and echo response using streams", async (cb) => {
    const script = utils.read(
      path.join(__dirname, "fixtures/worker-streams-cf.js")
    );
    const server = new Cloudworker(script).listen(8080);
  ...
  test("github issue 32", async (cb) => {
    const script = utils.read(
      path.join(__dirname, "fixtures/github-issue-32.js")
    );
    const server = new Cloudworker(script).listen(8080);
    const res = await axios.get("http://localhost:8080", defaultAxiosOpts);
    expect(res.status).toEqual(200);
  ...
  test("github issue 33", async (cb) => {
    const script = utils.read(
      path.join(__dirname, "fixtures/github-issue-33.js")
    );
    const server = new Cloudworker(script).listen(8080);
    const res = await axios.get("http://localhost:8080", defaultAxiosOpts);
    expect(res.status).toEqual(200);
  ...
  test("simple sha256 encode", async (cb) => {
    const script = utils.read(
      path.join(__dirname, "fixtures/sha256-encode.js")
    );
    const server = new Cloudworker(script).listen(8080);
    const res = await axios.get("http://localhost:8080", defaultAxiosOpts);
    expect(res.status).toEqual(200);
  ...
  test("simple hmac sign", async (cb) => {
    ...
    expect(res.status).toEqual(200);
    expect(res.data).toEqual(
      "http://localhost:8080/?mac=ae0342a9b989d76cfc457439f615ed72651ba7bce1994361621a003f6ade5508&expiry=1362078000000"
    );
    await server.close();
    cb();
  });
  ...
  test("simple hmac verify", async (cb) => {
    ...
    const res = await axios.get(
      "http://localhost:8080/verify/bob?mac=ae0342a9b989d76cfc457439f615ed72651ba7bce1994361621a003f6ade5508&expiry=1362078000000",
      defaultAxiosOpts
    );
  ...
  test("test cloudflare generate and verify recipes", async (cb) => {
    const cf_verify_script = utils.read(
      path.join(__dirname, "fixtures/cf-verify.js")
    );
    const cf_generate_script = utils.read(
      path.join(__dirname, "fixtures/cf-generate.js")
    );
    const server = new Cloudworker(cf_generate_script).listen(8080);
    const res = await axios.get(
      "http://localhost:8080/generate/bob",
      defaultAxiosOpts
    );
    expect(res.status).toEqual(200);
