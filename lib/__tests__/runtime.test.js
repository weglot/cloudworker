...
describe("runtime", () => {
  test("context allows overriding of properties via bindings", async () => {
    ...
    const context = new runtime.Context(() => {}, dummyFactory, {
      fetch: someMockFetch,
    });
  ...
  test("context has expected properties", async () => {
    ...
    const dummyFactory = Symbol("dummy factory");
    const got = Object.getOwnPropertyNames(
      new runtime.Context(() => {}, dummyFactory)
    ).sort();
  ...
  test("instanceof from different realm works", () => {
    ...
    const objTest = vm.runInNewContext(
      "instanceOfObject(new Object())",
      context
    );
    const arrayBufferViewTest = vm.runInNewContext(
      "isArrayBufferView(new Uint16Array())",
      context
    );
