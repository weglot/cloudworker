...
program
  ...
  .option(
    "-s, --kv-set [variable.key=value]",
    "Binds variable to a local implementation of Workers KV and sets key to value",
    collect,
    []
  )
  .option(
    "-f, --kv-file [variable=path]",
    "Set the filepath for value peristence for the local implementation of Workers KV",
    collect,
    []
  )
  .option(
    "-w, --wasm [variable=path]",
    "Binds variable to wasm located at path",
    collect,
    []
  )
  ...
  .option(
    "-r, --watch",
    "Watch the worker script and restart the worker when changes are detected",
    false
  )
  .option(
    "-s, --set [variable.key=value]",
    "(Deprecated) Binds variable to a local implementation of Workers KV and sets key to value",
    collect,
    []
  )
  .option(
    "--tls-key <tlsKey>",
    "Optional. Path to encryption key for serving requests with TLS enabled. Must specify --tls-cert when using this option."
  )
  .option(
    "--tls-cert <tlsCert>",
    "Optional. Path to certificate for serving requests with TLS enabled. Must specify --tls-key when using this option."
  )
  .option(
    "--https-port <httpsPort>",
    "Optional. Port to listen on for HTTPS requests. Must specify --tls-cert and --tls-key when using this option. May not be the same value as --port.",
    3001
  )
  .action((f) => {
    file = f;
...
function run(file, wasmBindings) {
  ...
  const bindings = utils.extractKVBindings(
    program.kvSet.concat(program.set),
    program.kvFile
  );
  Object.assign(bindings, wasmBindings);

  // Add a warning log for deprecation
  if (program.set.length > 0)
    console.warn(
      "Warning: Flag --set is now deprecated, please use --kv-set instead"
    );

  if (
    (program.tlsKey && !program.tlsCert) ||
    (!program.tlsKey && program.tlsCert)
  ) {
